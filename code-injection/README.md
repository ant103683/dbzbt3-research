...
## Requirements
* Any hex editor out there (in this case, 010 Editor)
* Cheat Engine
* DBZBT3 Unused Functions CSV
* DBZBT3 Unused Methods CSV
* Ghidra
* PCSX2
...
# Brief Explanation
This means of code injection does NOT add additional binaries or code to the main ELF (Executable and Linkable Format, in this case ``slps_258.15``).
...
In this case, the initial bytes are ``F0 FF BD 27 00 00 BF FF``.

That is clearly not enough, because 1062 other functions/methods start with that exact sequence.
...
The same logic can be applied to finding the function/method directly in the ELF file.

In the ELF of the NTSC-U version, slus_216.78, the method is located at 0x000DD480.
...
## Try It Out!
1. Added Ring-Out to 1 map (Mountain Road - Evening)
2. Added Ring-Out to 4 maps (all Desert maps, Planet - Night, and Mountain Road - Evening)
## Demonstration
